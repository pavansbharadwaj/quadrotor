...
nonlinear = [	vx;
							...
							wy;
							wz;
							 Jy*wy*wz - Jz*wy*wz - (2^(1/2)*d*(ctrl1 + ctrl2 - ctrl3 - ctrl4))/2/Jx
							Jz*wx*wz - Jx*wx*wz - (2^(1/2)*d*(ctrl1 - ctrl2 - ctrl3 + ctrl4))/2/Jy
							Jx*wx*wy - (b*(ctrl1 - ctrl2 + ctrl3 - ctrl4))/k - Jy*wx*wy/Jz
							];
...
Q =[600*ones(1,2)  400*ones(1,2) 10^-4].*eye(length(A(1,:))); % eye(length(A(:,1)));
R = eye(length(B(1,:)));
N = 0;

[K,~,~] = dlqr(A_d, B_d, Q, R, N);

disp("done")


thrust_conversion= 1/(0.06/4*9.81/65536);
angle_conversion = pi/180;

disp(K*thrust_conversion*angle_conversion)